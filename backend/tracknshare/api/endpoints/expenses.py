...
def get_expenses(skip: int = 0, limit: int = 10, db: Session = Depends(get_db.get_db), user: User = Depends(dependencies.get_current_user)):
...
def add_expense(expense_to_add: ExpenseCreate, db: Session = Depends(get_db.get_db)):
